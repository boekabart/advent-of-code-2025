...
internal record Point3D(long X, long Y);
...
public static class D9P1
{
    public static object Part1Answer(this string input)
    {
        ...
        var pQueue = QueueByDistance(positions, 1);

        var pair = pQueue.Dequeue();
        return pair.one.SquareSize(pair.two);
    }

    internal static PriorityQueue<(Point3D one, Point3D two), double> QueueByDistance(this List<Point3D> positions,
        int n = 0)
    {
    ...
    internal class Hall
    {
        ...
        public Hall(List<Point3D> positions)
        {
            ...
            PosByCircuit = positions.WithIndex()
                .ToDictionary(pair => pair.index, pair => new List<Point3D> { pair.item });
        }
    ...
    extension(Point3D input)
    {
        internal long SquareSize(Point3D other)
            => (1 + Math.Abs(input.X - other.X)) *
               (1 + Math.Abs(input.Y - other.Y));
    }
    ...
    extension(string input)
    {
        ...
        internal Point3D? TryParseAsPoint()
        {
            var s = input.Split(',');
            if (s.Length == 2)
                return new(long.Parse(s[0]), long.Parse(s[1]));
            return null;
        }
    }
}